...
export const ComputerGameModel = () => {
  const router = useRouter();
  const [isMounted, setIsMounted] = useState(false);
  const { isOpen, onClose, aiChatResult, type } = useModal();
  const isModelOpen = isOpen && type === "computergameModel";
      ...
      <Dialog open={isModelOpen} onOpenChange={onClose}>
        ...
        >
          <DialogHeader className="pt-4 px-6">
            ...
            <h1 className=" text-center text-white font-bold text-xl">Play With Computer</h1>
            <span><Computer className=" w-8 h-8 text-white ml-5"></Computer></span>
            </div>
